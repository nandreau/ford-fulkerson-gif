...
if __name__ == '__main__':
    ...
    image_paths = []
    frame_duration = 1000
    for i, g in enumerate(obj.augPathsGraph):
        png_data = g.create_png()
        image = Image.open(BytesIO(png_data))
        image_paths.append(image)

    frame_durations = [frame_duration] * len(image_paths)
    image_paths[0].save("output.gif", save_all=True, append_images=image_paths[1:], optimize=False, loop=0, duration=frame_durations)
